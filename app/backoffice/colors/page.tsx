    ...
    return (
        <div className="px-10 py-10">
            <div className="flex items-center justify-between mb-10">
                <div>
                    <h1 className="font-black text-[32px] uppercase tracking-tight">Couleurs</h1>
                    <p className="text-white/40 text-[13px] mt-1">{colors.length} couleur{colors.length > 1 ? 's' : ''}</p>
                </div>
                <button
                    onClick={() => { setAdding(true); setEditing(null); setForm({ name: '', hex: '#000000', price: 0 }) }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {loading ? (
                    ...
                    <div key={color.id} className="bg-white/5 border border-white/20 rounded-xl p-5 flex items-center gap-4">
                        <div
                            className="w-12 h-12 rounded-full flex-shrink-0 border-2 border-white/10"
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                ...
                                className="text-[11px] px-3 py-1.5 rounded bg-white/20 text-white/60 hover:bg-white/15 hover:text-white transition-colors"
                            >
                                Éditer
                            </button>
                            <button
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-[#111] border border-white/10 rounded-2xl p-8 w-full max-w-md z-10">
