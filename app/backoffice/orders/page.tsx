                                ...
                                <tr key={order.id} className="border-b border-white/5">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="relative">
                                            ...
                                            {openId === order.id && createPortal(
                                                <div
                                                    className="fixed text-white bg-[#111] border border-white/10 rounded-lg shadow-xl overflow-y-auto"
                                                    style={{
                                                        top: `${document.getElementById(`order-${order.id}`)?.getBoundingClientRect().bottom}px`,
                                                        left: `${document.getElementById(`order-${order.id}`)?.getBoundingClientRect().left}px`,
                                                        width: `${document.getElementById(`order-${order.id}`)?.getBoundingClientRect().width}px`,
                                                        zIndex: 9999
                                                    }}
                                                >
                                                    {STATUSES.map(s => (
